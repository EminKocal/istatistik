...
logs ( client );
let pub = "785315028935901195"; ////// kategori id
let kayıt = ""
let terapi = ""
let sorun = ""
let vk = ""
...
client.on ( "message" , async msg => {
    if ( msg.content.startsWith ( "!stat" ) || msg.content.startsWith ( "!me" ) ) {
        ...
        if(!['781999973435834368'].some(role => msg.member.roles.cache.get(role)) && !msg.member.hasPermission('ADMINISTRATOR')) return msg.react('❌')
        let st = member.voice
...
client.on ( "message" , async msj => {
    ...
    if(!['781999973435834368'].some(role => msj.member.roles.cache.get(role)) && !msj.member.hasPermission('ADMINISTRATOR')) return msj.react('❌')
...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Emin Stat "}, status: "dnd" }); //dnd rahatsı etmeyın, online çevirimiçi, idle boşta 
});

client.login ("NzgwMDQ5ODgzNTI0NjI4NTAx.X7pcEA.4x-bI6HWLWmJmNnB0MEVRQbmMwQ");